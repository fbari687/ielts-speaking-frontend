...
const handleSubmit = async () => {
  ...
  try {
    ...
    const message = error.response?.data?.message || "Submission failed. Please check your backend connection.";
...
<template>
  <div class="min-h-screen bg-slate-50 text-slate-800 py-8 px-4 sm:px-6 lg:px-8">
    <div class="max-w-6xl mx-auto space-y-6">
      <!-- Navigation / App Bar -->
      <div class="flex flex-col sm:flex-row items-center justify-between bg-white border border-slate-200 rounded-lg p-4 shadow-sm gap-4">
        <div>
          <h1 class="text-xl font-bold tracking-tight text-slate-900">IELTS Speaking Evaluation</h1>
          <p class="text-xs text-slate-500">
            Logged in as <span class="font-semibold text-slate-700">{{ currentUser?.name || "User" }}</span> ({{ currentUser?.email }})
          </p>
        </div>
        <Button label="Sign Out" icon="pi pi-sign-out" severity="secondary" size="small" outlined :loading="isLoggingOut" @click="handleLogout" />
      </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <Card class="lg:col-span-7 shadow-sm border border-slate-200">
          <template #title>
            <span class="text-lg font-semibold text-slate-800">Your Speaking History</span>
          </template>
          <template #content>
            <DataTable :value="attempts" :loading="isLoadingAttempts" responsiveLayout="scroll" emptyMessage="No speaking attempts recorded yet for this account." class="pt-2 text-sm">
